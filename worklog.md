...
Task ID: 15
Agent: Super Z (main agent)
Task: "Review the current codebase and implementation, troubleshoot potential issues, and continue improving the engineering details." — third full review round (first over the Task-14 heatmap code).

Work Log:
- Re-read every engine + UI file end to end (activity, storage ledger, profiles, memory, generator, pool, online-pack, use-typing-session, page, audit, insights consumers, results, stats-panel, word-display, result-chart, history-chart, key-heatmap, settings-modal, command-palette, connection-badge, layout).
- BUG #1+2 (real, DST — found by writing a probe BEFORE fixing): day math in the new heatmap used 86400000ms arithmetic. Under TZ=America/New_York, scripts/probe-dst.ts proved (a) 34/53 grid columns land off-midnight — fall-back columns land 23:00 the PREVIOUS day, shifting cell date keys for the rest of the grid; (b) activitySummary's exact `ms - prevMs === 86400000` run check breaks every streak crossing a fall-back (days are 25h long); (c) the currentRun backward walk is fragile the same way. Fix: calendar arithmetic everywhere — setDate()-based column/cell generation in activity-heatmap.tsx, a noon-anchored dayNum() calendar-day diff in activity.ts, setDate walk for currentRun, and the buildActivityDays window filter now compares calendar day-keys instead of raw ms (no 23:59/00:01 boundary flicker). Verified: probe-dst 9/9 under America/New_York, Asia/Kolkata (+5:30), Pacific/Lord_Howe (half-hour DST); activity-check still 40/40; grid spot-checks (firstMonday midnight+Monday, transition-week row 6 = Sun Nov 2).
- BUG #3 (display correctness for heavy users): audit compare.testNo used stats.history.length+1, which stalls at 501 forever once history hits its MAX_HISTORY=500 cap. Now Math.max(history.length+1, learning.totalTests) — the uncapped lifetime counter (already incremented by finalizeLearning before buildTestAudit runs). audit-check gained a capped-history assertion (847 tests -> testNo 847).
- CLEANUP (dead scaffold landmine): deleted src/lib/db.ts (instantiates PrismaClient at import in a strictly local-first app — one accidental import away from a crash) and src/app/api/route.ts (hello-world scaffold, zero callers). Prisma deps + schema.prisma left in package.json intentionally (removal is tooling churn with zero runtime benefit — flagged for a future housekeeping pass). Confirmed: no references to either file; /api now 404s; home still 200.
- Reviewed-and-left (documented, not churned): profiles.ingestEvents vs audit.analyzeEvents latency-sample populations differ by one rule (profiles counts a correct press after a wrong one; audit requires prevCorrect) — both defensible, not user-visible as wrong; result-chart "pb" label ends ~1px past the viewBox — invisible; sampleFlow's ultra-small-pool fallback loop lacks a repeat guard but its pools are 6.3k+ (dead-defensive path); handleReset keeps pack caches by design (generic vocabulary ≠ personal data).
- Verification: activity-check 40/40, probe-dst 9/9 (x3 timezones), audit-check ALL PASS (+1 new assertion), check-insights PASS, check-boundaries ALL PASS, check-generator 68/68, tsc filtered-clean, eslint clean on touched files, smoke-activity.sh PASS end-to-end (GRID-OK, SUMMARY-OK, today marked), home 200 + /api 404 + dev.log clean.
- Files: src/lib/typing/activity.ts (calendar day math), src/components/typing/activity-heatmap.tsx (calendar grid), src/lib/typing/audit.ts (lifetime testNo), scripts/probe-dst.ts (NEW), scripts/audit-check.ts (+1 assertion). Deleted: src/lib/db.ts, src/app/api/route.ts.

Stage Summary:
- The review's headline find: the Task-14 heatmap (and its run/summary math) silently mis-rendered for anyone in a DST timezone — a class of bug invisible in the Manila dev environment and caught only by probing under foreign TZs. All day arithmetic now uses calendar operations verified across three timezone shapes (full DST, +30 offset, half-hour DST). Also fixed the audit's test counter stalling at the history cap and removed dead backend scaffold that contradicted the local-first architecture (and could crash on a future import).
- Files: see Work Log.

---
Task ID: 16
Agent: Super Z (main agent)
Task: "track worst/error and fastest words; error-prone words reappear often-but-not-too-often, fast/easy words get suspended for rounds scaled by difficulty & speed; words serving as key-drill vehicles get lower suspension" — user asked for SOTA instead if better. Constraint: skip anything already implemented or superseded by a better existing alternative.

Work Log:
- Redundancy audit first (per user constraint): keys/bigrams ALREADY run FSRS-5 (memory.ts/ts-fsrs, 1m/8m learning steps), motor priors + Bayesian shrinkage (motor.ts/effLatency), and set-cover curation already re-serves words containing weak keys regardless of any word state. Verdict: the only genuinely missing layer is per-WORD memory. No second scheduler written; the word layer reuses the existing ts-fsrs machinery (reviewMem/memRetrievability/sanitizeMem) end to end.
- types.ts: WordOutcome (target/typed/ms/errKeys/partial — captured by the hook because backspaces + word boundaries are unrecoverable from the event log) + WordProfile (attempts/errors/bestWpm/lastSeen/mem) + LearningData.wordProfiles; lastVersion 3 -> 4 (page.tsx boot-migration writes back once).
- NEW src/lib/typing/word-scheduler.ts — pure module: normalizeWordKey (strips generator punctuation transforms, rejects numbers/>16 chars), expectedWordMs (Bayesian yardstick: shrunk bigram latency -> shrunk key latency -> classifyBigram prior per transition, same-key repeats like "iii" get a 0.75x discount, final space interval = personal median), ingestWordOutcomes (skips partial/number tokens; bestWpm only from PERFECT attempts), finalizeWordReviews (one FSRS review per word per test; grade: imperfect diff -> again, recovered slips (errKeys>0, clean diff) -> hard, clean-but-slower-than-expected -> hard, clean+fast with history -> easy, first-review Easy capped at good), wordUrgencies (mirrors keyUrgencies), pickReviewWords (floor+jitter+pool filter+cap), worstWords/fastestWords/dueWords rankings.
- CALIBRATION FIX (caught by own checks before shipping): word error rates must be measured against a WORD prior (~16% = 1-(1-3.5%)^5), not the per-keystroke PRIOR_BASE_ERR — with the keystroke prior every healthy word looked chronically broken (shrunk 2/2-error word -> urgency 2.56, served forever). WORD_PRIOR_ERR=0.16 + K=6 -> cold 2-for-2 word lands at 0.199, below the 0.28 serving floor.
- generator.ts: adaptive mode gains the word-review channel — FSRS-due words reserve up to 45% (hard cap 8) of the drill budget before greedyCover runs; suspension applies ONLY to this channel. Cover still ignores word mem (words needed for weak-key drills are served on key merit = the "lower suspension for in-test words" requirement, now principled), flow stays unscheduled (connective tissue).
- use-typing-session.ts: per-attempt tracking (wordStart/wordErr refs reset on submit + on both pull-back paths, outcomes index-aligned with typedWords and popped in lockstep); finishTest commits the in-progress word with partial flag (exact-match auto-finish = complete; timer cutoff = skipped by scheduler). onFinish gains the wordOutcomes param.
- page.tsx handleFinish: ingestWordOutcomes + finalizeWordReviews alongside the existing key/bigram finalize.
- storage.ts: sanitizeLearning hardens wordProfiles (strict /^[a-z][a-z'-]{0,15}$/ keys, errors clamped to attempts, Infinity bestWpm -> null, missing mem reseeded from lifetime stats, 4000-entry ceiling keeping the most recent), lastVersion 4.
- stats-panel.tsx: new "words" tab — tracked-word count + explanation line, WORST WORDS (error rate, errors/attempts chips, red), QUEUED FOR REVIEW (urgency order with "due in 5m"-style FSRS countdown), FASTEST WORDS (best clean wpm); Radix Tabs note: programmatic activation needs the full pointerdown/mousedown/.../click sequence, bare .click() silently no-ops.
- Browser E2E (scripts/smoke-words.sh): typed a live 10-word adaptive test with one deliberately failed word -> wordProfiles built for all 10, failed word graded again (mem reps 1, due +58s = the 1m learning step), clean 8-char word graduated to the 8m step (due +478s); words tab live shows "queued for review" with the failed word; seeded 3-test profile lights all three sections + chips (screenshot scripts/words-tab.png); Tab-restart after scheduling still renders. localStorage cleaned.

Verification: NEW scripts/word-check.ts 50/50 (normalize, expected-ms incl. iii-discount + personal-latency uptake, tally/partial/number skips, grade mapping incl. first-review Easy cap, suspension dynamics (fresh-easy 0.012 vs floor 0.28; failed-word resurfacing after the 1m step), pool/jitter/cap serving, rankings, sanitize junk/clamp/reseed/4000-ceiling, export/import round-trip, 12-run generateAdaptive integration: due 'quiet' served 12/12, mastered 'calm' 0/12). Full chain re-run: check-generator 68/68, check-insights PASS, check-boundaries ALL PASS, audit-check ALL PASS, activity-check 40/40 (one fixture literal gained wordProfiles), tsc filtered-clean, eslint clean on all 10 touched files, dev.log clean after the mid-edit HMR window (16 transient PRIOR_BASE_LAT errors while word-scheduler.ts was between writes — resolved by the final compile, confirmed by all-green smoke).

Stage Summary:
- Cadence now remembers WORDS: every committed attempt feeds a per-word FSRS card, so the adaptive test you get next contains your recently-failed and chronic words within ~1-3 tests, backs clean-fast words off for days, and still drills weak keys on key merit. Suspension is one channel of a three-channel design (review=FSRS-scheduled, cover=key-merit, flow=unscheduled) — no conflict with the existing scheduler, which is reused verbatim.
- Files: src/lib/typing/word-scheduler.ts (NEW), types.ts (+WordOutcome/+WordProfile/+wordProfiles v4), profiles.ts (+wordProfiles seed, effLatency exported), generator.ts (review channel), storage.ts (wordProfiles sanitize/ceiling), use-typing-session.ts (outcome capture), page.tsx (ingest+finalize), stats-panel.tsx (words tab). New tools: scripts/word-check.ts, scripts/smoke-words.sh, scripts/probe-wordtab.ts (+words-tab.png).
